...
class Products_model extends CI_Model
{
	...
	{
		$q = $this->db->get_where ( 'warehouses', array (
				'id' => $id 
		), 1 );
		if ($q->num_rows () > 0)
		{
	...
	public function updateProduct($id, $photo, $data = array())
	{
		...
		if ($this->db->update ( 'products', $productData ))
		{
			...
			editProductsPush ( $id, $productData );
			return true;
		} else
		{
			return false;
...
return true;
	...
	{
		$q = $this->db->get_where ( 'categories', array (
				'id' => $id 
		), 1 );
		if ($q->num_rows () > 0)
		{
	...
	{
		$q = $this->db->get_where ( 'tax_rates', array (
				'name' => $name 
		), 1 );
		if ($q->num_rows () > 0)
		{
	...
	{
		$q = $this->db->get_where ( "subcategories", array (
				'category_id' => $category_id 
		) );
		if ($q->num_rows () > 0)
		{
	...
	{
		$q = $this->db->get_where ( 'damage_products', array (
				'id' => $id 
		), 1 );
		if ($q->num_rows () > 0)
		{
	...
	public function addDamage($product_id, $date, $quantity, $warehouse, $note)
	{
		...
		if ($this->db->insert ( 'damage_products', $data ) && $this->db->update ( 'products', array (
				'quantity' => $nQTY 
		), array (
				'id' => $product_id 
		) ))
	...
	public function updateDamage($id, $product_id, $date, $quantity, $warehouse, $note)
	{
		...
		if ($this->db->update ( 'damage_products', $data, array (
				'id' => $id 
		) ) && $this->updateQuantity ( $product_id, $warehouse, $balance_qty ) && $this->db->update ( 'products', array (
				'quantity' => $nQTY 
		), array (
				'id' => $product_id 
		) ))
	...
	public function updateQuantity($product_id, $warehouse_id, $quantity)
	{
		...
		if ($this->db->update ( 'warehouses_products', $productData, array (
				'product_id' => $product_id,
				'warehouse_id' => $warehouse_id 
		) ))
		{
	...
	{
		if ($this->db->delete ( 'damage_products', array (
				'id' => $id 
		) ))
		{
			return true;
