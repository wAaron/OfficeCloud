...
class Products extends MX_Controller
{
	...
	{
		$this->load->library ( 'datatables' );
		$this->datatables->select ( "products.id as productid, products.image as image, products.code as code, products.name as name, categories.name as cname, products.price, COALESCE(quantity, 0) as quantity, products.unit, alert_quantity", FALSE );
	...
	function getwhproducts($warehouse_id = NULL)
	{
		...
		$this->load->library ( 'datatables' );
		$this->datatables->select ( "warehouses_products.warehouse_id as wh, warehouses_products.product_id as productid, products.image as image, products.code as code, products.name as name, products.unit, products.price, warehouses_products.quantity, alert_quantity", FALSE )->from ( 'warehouses_products' )->join ( 'products', 'products.id=warehouses_products.product_id', 'left' )->where ( 'warehouses_products.warehouse_id', $warehouse_id )->where ( 'warehouses_products.quantity !=', 0 )->group_by ( "products.id" )->add_column ( "Actions", "<center><a id='$4 - $3' href='index.php?module=products&view=gen_barcode&code=$3&height=200' title='" . $this->lang->line ( "view_barcode" ) . "' class='barcode tip'><i class='icon-barcode'></i></a>
		...
		unset_column ( 'productid' )->unset_column ( 'wh' )->unset_column ( 'image' );
	...
	function getseatchresults($srq = NULL)
	{
		...
		$this->load->library ( 'datatables' );
		$this->datatables->select ( "products.id as productid, image, code, name, unit, cost, price, (CASE WHEN sum(warehouses_products.quantity) Is Null THEN 0 ELSE sum(warehouses_products.quantity) END) as totalQuantity, alert_quantity", FALSE )->from ( 'products' )->like ( 'name', $srq, 'both' )->or_like ( 'code', $srq, 'both' )->join ( 'warehouses_products', 'products.id=warehouses_products.product_id', 'left' )->group_by ( "products.id" )->add_column ( "Actions", "<center><a href='#' onClick=\"MyWindow=window.open('index.php?module=products&view=barcode&code=$3', 'MyWindow','toolbar=no,location=no,directories=no,status=no,menubar=no,scrollbars=yes,resizable=yes,width=425,height=275'); return false;\">
		...
		unset_column ( 'productid' )->unset_column ( 'image' );
	...
	{
		$this->load->library ( 'datatables' );
		$this->datatables->select ( "damage_products.id as did, damage_products.product_id as productid, damage_products.date as date, products.image as image, products.code as code, products.name as pname, damage_products.quantity as quantity, warehouses.name as wh," );
		...
		$this->datatables->add_column ( "Actions", "<center><a href='index.php?module=products&view=edit_damage&product_id=$1&id=$2' class='tip' title='" . $this->lang->line ( "edit_damage_details" ) . "'><i class='icon-edit'></i></a>
